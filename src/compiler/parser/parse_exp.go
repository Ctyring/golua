...
*/
func parseExp(l *Lexer) Exp {
	return parseExp12(l)
}

// x or y
func parseExp12(l *Lexer) Exp {
	// 先解析更高优先级的运算符表达式
	exp := parseExp11(l)
	for l.LookAhead() == TOKEN_OP_OR { // 左结合，直接for遍历
		line, op, _ := l.NextToken()
		lor := &BinopExp{line, op, exp, parseExp11(l)}
...
// x and y
func parseExp11(l *Lexer) Exp {
	exp := parseExp10(l)
	for l.LookAhead() == TOKEN_OP_AND {
		line, op, _ := l.NextToken()
		land := &BinopExp{line, op, exp, parseExp10(l)}
...
func parseExp10(l *Lexer) Exp {
	...
	for {
		...
		case TOKEN_OP_LT, TOKEN_OP_GT, TOKEN_OP_NE,
			...
			line, op, _ := l.NextToken()
			exp = &BinopExp{Line: line, Op: op, Exp1: exp, Exp2: parseExp9(l)}
		default:
			return exp
		}
	}
...
// x | y
func parseExp9(l *Lexer) Exp {
	exp := parseExp8(l)
	for l.LookAhead() == TOKEN_OP_BOR {
		line, op, _ := l.NextToken()
		bor := &BinopExp{line, op, exp, parseExp8(l)}
...
// x ~ y
func parseExp8(l *Lexer) Exp {
	exp := parseExp7(l)
	for l.LookAhead() == TOKEN_OP_BXOR {
		line, op, _ := l.NextToken()
		bxor := &BinopExp{line, op, exp, parseExp7(l)}
...
// x & y
func parseExp7(l *Lexer) Exp {
	exp := parseExp6(l)
	for l.LookAhead() == TOKEN_OP_BAND {
		line, op, _ := l.NextToken()
		band := &BinopExp{line, op, exp, parseExp6(l)}
...
// shift
func parseExp6(l *Lexer) Exp {
	exp := parseExp5(l)
	for {
		switch l.LookAhead() {
		case TOKEN_OP_SHL, TOKEN_OP_SHR:
			line, op, _ := l.NextToken()
			shx := &BinopExp{line, op, exp, parseExp5(l)}
...
// x +/- y
func parseExp4(l *Lexer) Exp {
	exp := parseExp3(l)
	for {
		switch l.LookAhead() {
		case TOKEN_OP_ADD, TOKEN_OP_SUB:
			line, op, _ := l.NextToken()
			arith := &BinopExp{line, op, exp, parseExp3(l)}
...
// *, %, /, //
func parseExp3(l *Lexer) Exp {
	exp := parseExp2(l)
	for {
		switch l.LookAhead() {
		case TOKEN_OP_MUL, TOKEN_OP_MOD, TOKEN_OP_DIV, TOKEN_OP_IDIV:
			line, op, _ := l.NextToken()
			arith := &BinopExp{line, op, exp, parseExp2(l)}
			exp = optimizeArithBinaryOp(arith)
		default:
			return exp
		}
...
// unary
func parseExp2(l *Lexer) Exp {
	switch l.LookAhead() {
	case TOKEN_OP_UNM, TOKEN_OP_BNOT, TOKEN_OP_LEN, TOKEN_OP_NOT:
		line, op, _ := l.NextToken()
		exp := &UnopExp{line, op, parseExp2(l)}
...
// x ^ y
func parseExp1(l *Lexer) Exp { // pow is right associative
	exp := parseExp0(l)
	if l.LookAhead() == TOKEN_OP_POW { // 乘方具有右结合性，需要递归调用自己解析后面的乘方运算符表达式(这里使用if)
		line, op, _ := l.NextToken()
		exp = &BinopExp{line, op, exp, parseExp2(l)}
	}
	return optimizePow(exp)
}

func parseExp0(l *Lexer) Exp {
	switch l.LookAhead() {
	case TOKEN_VARARG: // ...
		line, _, _ := l.NextToken()
...
func parseNumberExp(l *Lexer) Exp {
	line, _, token := l.NextToken()
...
func parseFuncDefExp(l *Lexer) *FuncDefExp {
...
func parseTableConstructorExp(l *Lexer) *TableConstructorExp {
	line := l.Line()
...
// 解析字段列表
func _parseFieldList(l *Lexer) (ks, vs []Exp) {
...
// 解析字段
func _parseField(l *Lexer) (k, v Exp) {
